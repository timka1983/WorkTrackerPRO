...
interface CorrectionModalProps {
  editingLog: { userId: string; date: string } | null;
  logs: WorkLog[];
  users: User[];
  machines: Machine[];
  onClose: () => void;
...
const CorrectionModal: React.FC<CorrectionModalProps> = ({
  ...
  const employee = users.find(u => u.id === editingLog.userId);
      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
         <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
               ...
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{editingLog.date} — {employee?.name}</p>
            </div>
                      ...
                      <div className="flex gap-3 h-28">
                         <div className="flex-1 flex flex-col gap-1">
                            ...
                            {log.photoIn ? (
                               <div 
                                ...
                                onClick={() => onPreviewPhoto(log.photoIn!)}
                               >
                         ...
                         <div className="flex-1 flex flex-col gap-1">
                            ...
                            {log.photoOut ? (
                               <div 
                                className="flex-1 rounded-xl border border-slate-100 overflow-hidden cursor-zoom-in group/photo relative"
                                onClick={() => onPreviewPhoto(log.photoOut!)}
                               >
