  ...
  return (
    <div className="fixed inset-0 z-[200] bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn">
      <div className="bg-white rounded-[2.5rem] shadow-2xl dark:shadow-slate-900/40 p-10 w-full max-w-sm border border-slate-200 text-center space-y-6">
        ...
        <div>
          <h3 className="text-xl font-black text-slate-900 dark:text-slate-50 uppercase tracking-tight mb-2">Обновите тариф</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium leading-relaxed">{reason}</p>
        </div>
        <div className="space-y-3">
          <button 
            onClick={onUpgrade}
            className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-2xl dark:shadow-slate-900/20 shadow-blue-200 hover:bg-blue-700 transition-all active:scale-95"
          >
            Посмотреть тарифы
          </button>
          <button 
            onClick={onClose}
            className="w-full py-4 bg-slate-100 text-slate-500 dark:text-slate-400 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-slate-200 transition-all"
          >
            Позже
          </button>
        </div>
