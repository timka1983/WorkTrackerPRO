...
export const EmployeePrintView = memo<EmployeePrintViewProps>(({
  ...
  return (
    <div id="employee-calendar-print" className="hidden print:block bg-white text-black p-2" style={{ width: '290mm', height: '200mm', fontFamily: 'serif' }}>
      <div className="flex justify-between items-start mb-2">
        <div className="w-24 opacity-80">
          <div className="text-[6px] font-bold mb-0.5">{calendarData.prevMonth.name} {calendarData.prevMonth.year}</div>
          <div className="grid grid-cols-7 gap-0 text-[5px]">
        ...
        <div className="text-center flex-1">
          <h1 className="text-4xl font-black tracking-tight leading-none mb-0.5" style={{ fontFamily: 'serif' }}>{calendarData.monthName} {calendarData.year}</h1>
          <p className="text-[8px] font-bold uppercase tracking-widest text-slate-500">Рабочий табель сотрудника: {user.name}</p>
        </div>
        <div className="w-24 opacity-80 text-right">
          <div className="text-[6px] font-bold mb-0.5">{calendarData.nextMonth.name} {calendarData.nextMonth.year}</div>
          <div className="grid grid-cols-7 gap-0 text-[5px] text-left">
            {['Пн','Вт','Ср','Чт','Пт','Сб','Вс'].map(d => <div key={d} className="font-bold">{d}</div>)}
        ...
        <thead>
          <tr>
            {['Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота', 'Воскресенье'].map(d => (
              <th key={d} className="border border-black p-1 text-[8px] font-bold uppercase tracking-widest bg-slate-50">{d}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {Array.from({ length: 6 }).map((_, rowIdx) => {
            const weekDays = Array.from({ length: 7 }).map((_, colIdx) => {
              ...
              return (
                <td key={colIdx} className="border border-black h-16 p-1 relative vertical-align-top">
                  ...
                  <div className="h-full flex flex-col justify-end items-center text-center pb-0.5">
                     {absence ? (
                       <div className="bg-slate-900 text-white px-1 py-0.5 rounded-sm text-[7px] font-black uppercase">
