      ...
      {user && (
        <>
          ...
          <aside className={`
            ...
            w-64 sm:w-20 ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:w-20' : 'sm:w-64'}
            ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full sm:translate-x-0'}
            no-print
          `}>
            <div className="flex items-center justify-between px-4 sm:px-0 lg:px-4 mb-6 sm:mb-8">
              <div className={`flex items-center gap-3 sm:justify-center ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:justify-center' : 'sm:justify-start'} w-full`}>
                <div className="bg-blue-600 text-white p-2 rounded-xl shadow-md shrink-0">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </div>
                <span className={`font-bold text-slate-900 sm:hidden ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:hidden' : 'sm:block'}`}>WorkTracker</span>
              </div>
              <button 
              ...
              {user.role === UserRole.EMPLOYER && setEmployerViewMode ? (
                employerTabs.map(tab => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center sm:justify-center ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:justify-center' : 'sm:justify-start'} gap-3 p-3 rounded-xl transition-all group relative ${
                        isActive 
                    ...
                    >
                      ...
                      <span className={`font-medium sm:hidden ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:hidden' : 'sm:block'} ${isActive ? 'font-semibold' : ''}`}>{tab.label}</span>
                ...
                <>
                  <button
                    ...
                    className={`flex items-center sm:justify-center ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:justify-center' : 'sm:justify-start'} gap-3 p-3 rounded-xl transition-all group ${
                  ...
                  >
                    ...
                    <span className={`font-medium sm:hidden ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:hidden' : 'sm:block'} ${employeeViewMode === 'control' ? 'font-semibold' : ''}`}>Управление</span>
                  </button>
                  ...
                  >
                    ...
                    <span className={`font-medium sm:hidden ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:hidden' : 'sm:block'} ${employeeViewMode === 'matrix' ? 'font-semibold' : ''}`}>Мой Табель</span>
                  </button>
                  ...
                  <button
                    ...
                    className={`flex items-center sm:justify-center ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:justify-center' : 'sm:justify-start'} gap-3 p-3 rounded-xl transition-all group text-slate-500 hover:bg-slate-50 hover:text-slate-900`}
                    title="Сменить PIN"
                  >
                    ...
                    <span className={`font-medium sm:hidden ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:hidden' : 'sm:block'}`}>Сменить PIN</span>
                  </button>
                </>
              )}
            </nav>
            ...
            <div className="mt-auto flex flex-col gap-2 px-3 sm:px-2 lg:px-3 pt-4 border-t border-slate-100">
              <button
                ...
                className={`hidden sm:flex items-center sm:justify-center ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:justify-center' : 'sm:justify-start'} gap-3 p-3 rounded-xl text-slate-500 hover:bg-slate-50 hover:text-slate-900 transition-all group ${user.role === UserRole.EMPLOYER ? 'hidden' : ''}`}
              ...
              >
                ...
                <span className={`font-medium sm:hidden ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:hidden' : 'sm:block'}`}>
                  Свернуть меню
                </span>
              </button>
              ...
              {hasAdminPermissions && (
                <button
                  ...
                  className={`flex items-center sm:justify-center ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:justify-center' : 'sm:justify-start'} gap-3 p-3 rounded-xl text-slate-500 hover:bg-slate-50 hover:text-slate-900 transition-all group`}
              ...
              <button
                onClick={onLogout}
                className={`flex items-center sm:justify-center ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:justify-center' : 'sm:justify-start'} gap-3 p-3 rounded-xl text-slate-500 hover:bg-rose-50 hover:text-rose-600 transition-all group`}
                title="Выйти"
              >
                <LogOut className="w-5 h-5 shrink-0 stroke-2 group-hover:scale-110 transition-transform" />
                <span className={`font-medium sm:hidden ${isSidebarCollapsed || user.role === UserRole.EMPLOYER ? 'sm:hidden' : 'sm:block'}`}>Выйти</span>
              </button>
            </div>
          </aside>
        </>
